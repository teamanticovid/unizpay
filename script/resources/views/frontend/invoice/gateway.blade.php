                            ...
                            <div class="mt-4">
                                <table class="table table-bordered mb-4">
                                    ...
                                    <tr>
                                        <th>{{ __("Amount") }}</th>
                                        <td>
                                            @if($invoice->currency_id !== $gateway->currency_id)
                                                {{ currency_format($amount, currency: $invoice->currency) }}  =
                                            @endif
                                                {{ convert_money_direct($amount, $invoice->currency, $gateway->currency, true) }}
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $invoice->amountForGateway = convert_money_direct($amount, $invoice->currency, $gateway->currency);
                                            @endphp
                                            {{ currency_format($gateway->charge + $invoice->amountForGateway, currency: $gateway->currency) }}
                                        </td>
                                    </tr>
                                    </tbody>
                                </table>

                                <form action="{{ route('frontend.invoice.payment', [$invoice->uuid, $gateway->id]) }}" method="post" role="form" class="form-light form">
                                    @csrf
